...
export default async function createRepoImage({ repositories }) {
    await repositories.forEach(async (repository) => {
        const image = emojiRegex().exec(repository.description) ? emojiRegex().exec(repository.description)[0] : "📦";

        const directory = path.join(process.cwd(), "out", repository.name);
        const canvas = createCanvas(1500, 600);
        const context = canvas.getContext("2d");

        const backgroundImage = await loadImage(path.join(process.cwd(), "assets", "background.png"));
        ...
        let description = repository.description ? repository.description.replace(emojiRegex(), '') : "No description provided.";
        ...
        Logger({ type: "success", message: `Image created: ${directory}/image.png` });
    });
